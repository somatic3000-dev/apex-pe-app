...
function peerMetrics(company, quotes = {}) {
    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">
            Fund I · Portfolio Operating System · Market Intelligence
          </div>
        </div>
      </div>
      ...
      <div className="market-grid" style={{ marginBottom: 20 }}>
        <div className="market-card">
          ...
          <div className="market-card-name" style={{ color: marketToneColor }}>
            {marketTone}
          </div>

          <div className="muted">Based on live watchlist average movement.</div>

          <div className="market-card-price" style={{ color: marketToneColor }}>
            {marketSummary.avgChange.toFixed(2)}%
          </div>

          <div className="muted">
            {marketSummary.gainers} gainers · {marketSummary.losers} losers ·{" "}
            {marketSummary.loaded} loaded
          </div>
        </div>
        ...
        <div className="market-card">
          ...
          >
            {peerAvgChange.toFixed(2)}%
          </div>

          <div className="muted">Peer market movement proxy.</div>
        </div>

        <div className="market-card">
          <div className="card-title">Market Read-Through</div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-title">Portfolio Peer Signals</div>

        <div className="market-grid">
          {companies.map((company) => {
            const peer = peerMetrics(company, quotes);
            ...
            return (
              <div className="market-card" key={company.id}>
                ...
                >
                  {peer.avgChange.toFixed(2)}%
                </div>

                <div style={{ color: peer.color, fontWeight: 800 }}>
                  {peer.signal}
                </div>

                <div className="muted" style={{ marginTop: 8 }}>
                  {peer.loaded} / {peer.peers.length} peers loaded
                </div>

                <div className="muted" style={{ marginTop: 8 }}>
                  {peer.peers.length > 0 ? peer.peers.join(" · ") : "No peers set"}
                </div>
              </div>
            );
          })}
      ...
      <div className="card">
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="table">
            ...
            <tbody>
              {companies.map((p) => {
                const peer = peerMetrics(p, quotes);
